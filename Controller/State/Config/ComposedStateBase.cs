    ...
    public abstract class ComposedStateBase : AppStateBase
    {
        ...
        private void OnComposedAppMessage(AppMessage msg) =>
            (msg.Type switch
            {
                ...
                AppMessageType.BackRequested =>
                    (Action)(() =>
                    {
                        // If a toast is up, treat Back as "dismiss toast" first
                        if (ToastHost.Children.Count > 0)
                        {
                            ToastHost.Clear();
                            RestoreMenuFocus();
                        }
                        else if (_spec.BackIntent is not null && (_spec.BackEnabled?.Invoke() ?? true))
                        {
                            EventHub.Publish(_spec.BackIntent());
                        }
                        HandleAppMessage(msg);
